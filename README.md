...
+ My virtualization environment as it is described below is currently contained/constrained to a single Lenovo Tiny M900 (not described below). The primary pool's host OSs were intentionally wiped for the first rebuild in several years to prepare for a series of video tutorials/training on XCP-ng and Xen Orchestra likely to be posted on YouTube (and linked here).

+ One of my goals beyond laying the virtualization foundation is to build in some level of container orchestration. For a while, I had been waffling about between Docker Swarm and some flavor of Kubernetes. There is already a somewhat decent level of Docker/Compose usage, so my first thought went to Docker Swarm. Of course, after spending just a small amount of time simply trying to search Google for which is better for my use case, far too many links would proclaim that Docker and/or Swarm is dead and this and that and this and that. Too much of it seemed like somewhere between incompetent analysis and kneejerk conspiracy theory. Fanatics gonna fanatic. It's like Genesis vs. SNES or PS2 vs. Xbox all over again, just in the container space. I don't have the experience to break down the truth here, but this [excellent post was revelatory](https://thedecipherist.com/articles/docker_swarm_vs_kubernetes/). I think it is safe to say that since Mirantis acquired Docker's enterprise business in 2019 there are many more competitors in the container space, but also that Docker Swarm is not going anywhere anytime soon. **tl;dr** Docker Swarm will be my choice for container orchestration, at least in the short term.

+ My time has been partly consumed by studying for technical certifications, so progress on rebuilding everything has been a bit slow lately. I don't know how popular LFCS and DCA actually are, but I figured starting with familiar is better than forcing myself back into the Cisco world for the first time since maybe 2013. Then maybe LPIC-1 and/or RHCSA. Eh, time will tell.
